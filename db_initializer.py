...
def _ensure_chat_tpl_message_columns(conn):
    """Widen chat message columns so catalog replies with [PRODUCT:…] tags can be stored."""
    cur = conn.cursor()
    alterations = (
        ('CHAT_TPL', 'LASTMESSAGE', 4000),
        ('CHAT_TPLDTL', 'MESSAGETEXT', 4000),
    )
    try:
        for table_name, column_name, size in alterations:
            try:
                conn.commit()
                cur.execute(
                    f'ALTER TABLE {table_name} ALTER COLUMN {column_name} TYPE VARCHAR({int(size)})'
                )
                conn.commit()
                print(f'[DB INIT] {table_name}.{column_name} widened to VARCHAR({size})')
            except Exception as exc:
                err = str(exc).lower()
                if 'unsuccessful metadata update' in err and 'same' in err:
                    continue
                if 'column unknown' in err or 'table unknown' in err:
                    continue
                print(
                    f'[DB INIT WARNING] Could not widen {table_name}.{column_name} to VARCHAR({size}): {exc}'
                )
    finally:
        cur.close()
...
def initialize_database(db_path, db_user, db_password):
    ...
    try:
        ...
        # Chat history: widen message columns on DBs created before MESSAGETEXT(4000)
        _ensure_chat_tpl_message_columns(conn)
